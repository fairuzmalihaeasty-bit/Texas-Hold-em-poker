...
func compareHandler(c *gin.Context) {
    ...
    h1h, err := parseCardSlice(req.Hand1.Hole); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
    h1c, err := parseCardSlice(req.Hand1.Community); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
    h2h, err := parseCardSlice(req.Hand2.Hole); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
    h2c, err := parseCardSlice(req.Hand2.Community); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }

    cr := usecase.CompareHandsDetailed(domain.Hand{Hole: h1h, Community: h1c}, domain.Hand{Hole: h2h, Community: h2c})
    winner := "tie"
    if cr.Winner == 1 { winner = "hand1" }
    if cr.Winner == -1 { winner = "hand2" }
...
func simulateHandler(c *gin.Context) {
    ...
    hero, err := parseCardSlice(req.Hero.Hole); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
    community, err := parseCardSlice(req.Community); if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
    if req.NumPlayers < 2 { req.NumPlayers = 2 }
